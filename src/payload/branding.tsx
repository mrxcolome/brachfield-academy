...
// no la marca de Payload. Tras editar este archivo: npx payload generate:importmap
...
function Squares({ size }: { size: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 96 96" aria-hidden="true" focusable="false">
      <rect x="4" y="10" width="38" height="38" rx="3" fill={NAVY} transform="rotate(-12 23 29)" />
    ...
    >
      ...
      >
        <span style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.01em' }}>
          Brachfield <span style={{ color: NAVY }}>Academy</span>
        </span>
        <span style={{ fontSize: 13, fontWeight: 500, color: ORANGE }}>Panel de contenido</span>
      </span>
    </span>
  )
}
...
export function Icon() {
  return (
    <span role="img" aria-label="Brachfield Academy" style={{ display: 'inline-flex' }}>
      <Squares size={26} />
    </span>
  )
}
